#!/usr/bin/env bash
# Helper library — the chunk config below calls only some of these.
# shellcheck disable=SC2329
set -u
INPUT="$(cat)"
export INPUT_JSON="$INPUT"

# ${#var} counts bytes, not characters, unless the ctype locale is UTF-8.
case "${LC_ALL:-${LC_CTYPE:-${LANG:-}}}" in *[Uu][Tt][Ff]*) ;; *) export LC_ALL=en_US.UTF-8 ;; esac

# Terminal width for flex-spacer math. Honors STATUSLINE_COLS env override
# (used by parity tests); otherwise asks the controlling terminal, because tput
# reports the terminfo default of 80 when stdout is a pipe, as it is here.
if [ -z "${STATUSLINE_COLS:-}" ]; then
	STATUSLINE_COLS=$(stty size </dev/tty 2>/dev/null | cut -d' ' -f2)
	case "${STATUSLINE_COLS:-0}" in '' | *[!0-9]* | 0) STATUSLINE_COLS=${COLUMNS:-0} ;; esac
	case "${STATUSLINE_COLS:-0}" in '' | *[!0-9]* | 0) STATUSLINE_COLS=$(tput cols 2>/dev/null || echo 80) ;; esac
	# Claude Code lays the status line out two columns narrower than the terminal.
	STATUSLINE_COLS=$((STATUSLINE_COLS - ${STATUSLINE_RESERVE:-2}))
	[ "$STATUSLINE_COLS" -gt 0 ] || STATUSLINE_COLS=1
fi

# Width as Claude Code measures it, which is what decides truncation: it bills
# Ambiguous-width glyphs (Nerd Font icons, …) at two columns even though the
# terminal draws them in one. macOS awk length() counts bytes, so ${#s} is used.
__visible_len() {
	local s p
	s="$(printf '%s' "$1" | sed 's/\x1b\[[0-9;]*m//g')"
	p="${s//[$'\U0000E000'-$'\U0000F8FF'$'\U000F0000'-$'\U000FFFFD'$'\U00002026']/}"
	printf '%s' $((${#s} + ${#s} - ${#p}))
}
